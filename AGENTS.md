...
本仓库为 **ledger v2** 的初始代码库，用于在保留 v1 参考实现的前提下，完成一次完整的技术与架构升级。重构目标包括：
...
新增或变更目录结构、环境变量、开发/构建/部署命令时，应同步更新 `README.md`，使新贡献者无需通读源码即可上手。若技术栈、全栈架构边界或 v1/v2 边界说明有变，应同时更新本文件。
